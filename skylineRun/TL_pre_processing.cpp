...
const long int BLK_SZ_PRE = 4194304;  //1024*1024*4 sizeof blk for I/O
const int SZ_VERTEX_PRE = sizeof(int);
const int VERTEX_PER_BLK_PRE = BLK_SZ_PRE / SZ_VERTEX_PRE;
typedef int vertex_id;

char* read_buff_1_pre;
...
bool *ficout, *ficin;
int *Q, *L, *R, *Lnew, *Qnew, *dag_pre;
int Hdegree[K], level, round_r;
char s[50], label_name_pre[50];
int tgraph_pre, tgraph2;
int pt_pre, pt2_pre;
int rN[max_round];
stack<int> dS;
...
void DAG() {
	...
	memset(dag_pre, -1, sizeof(int) * vertexN);
	...
	for (char *i = dag_l; *i != '\0'; i++) {
		label_name_pre[tgraph2++] = *i;
	}
	label_name_pre[tgraph2] = '\0';
	FILE *bfile = fopen(label_name_pre, "wb+");
	fwrite(dag_pre, SZ_VERTEX_PRE, vertexN, bfile);
	fclose(bfile);
	free(dag_pre);
}
...
void topoSort_B() {  //original graph's topological level
	...
	label_name_pre[tgraph2] = '\0';
	FILE * bfile1 = fopen(label_name_pre, "wb+");
	fwrite(topo, SZ_VERTEX_PRE, vertexN, bfile1);
	fclose(bfile1);
	free(Q_B);
	free(topo);
}
...
void creatM() {
	...
	dag_pre = (int *)malloc(sizeof(int) * (vertexN));
...
void TLabel() {
	...
	label_name_pre[tgraph2] = '\0';
	FILE * bfile2 = fopen(label_name_pre, "wb+");
	vertex_id * write_buff = (vertex_id *)malloc(BLK_SZ_PRE);
	int ptr_buff = 0;
	for (int i = 0; i < 2 * vertexN; i++) {
		for (int j = 0; j < SL[i].size(); j++) {
			write_buff[ptr_buff++] = SL[i][j];  //arrSL[i][j];
			if (ptr_buff == VERTEX_PER_BLK_PRE) {
				fwrite(write_buff, SZ_VERTEX_PRE, ptr_buff, bfile2);
				ptr_buff = 0;
			}
		}
	}
	...
	label_name_pre[tgraph2] = '\0';
	FILE * bfile3 = fopen(label_name_pre, "wb+");
	for (int i = 0; i < 2 * vertexN; i++) {
		write_buff[ptr_buff++] = SL[i].size();  //arrSL[i][j];
		if (ptr_buff == VERTEX_PER_BLK_PRE) {
			fwrite(write_buff, SZ_VERTEX_PRE, ptr_buff, bfile3);
			ptr_buff = 0;
		}
...
void transform() {
	vector<int> a1[round_r], a2[round_r], a3[round_r], a4[round_r], a5[round_r];
	for (int i = 0; i < round_r; i++)
	{
		rN[i] = deleV[i].size();
	}
	SLin1 = (int **)malloc(round_r * sizeof(int *));
	for (int i = 0; i < round_r; i++) {
	...
	SLout1 = (int **)malloc(round_r * sizeof(int *));
	for (int i = 0; i < round_r; i++) {
	...
	deleV1 = (int **)malloc(round_r * sizeof(int *));
	for (int i = 0; i < round_r; i++) {
	...
	startSLout1 = (int **)malloc(round_r * sizeof(int *));
	for (int i = 0; i < round_r; i++) {
	...
	startSLin1 = (int **)malloc(round_r * sizeof(int *));
	for (int i = 0; i < round_r; i++) {
